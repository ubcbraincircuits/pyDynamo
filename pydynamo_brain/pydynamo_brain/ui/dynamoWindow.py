...
class DynamoWindow(QtWidgets.QMainWindow):
    def __init__(self, app, argv):
        ...
        if len(argv) == 1:
            ...
            elif fileToOpen.endswith(".tif") or fileToOpen.endswith(".tiff") or fileToOpen.endswith(".czi") or fileToOpen.endswith(".lsm"):
    ...
    def toggleRadiiMode(self, originWindow):
        if not self.fullState.inRadiiMode():
            ...
            self.updateStatusMessage(None)
        self.fullActions.toggleRadiiMode()
        self.redrawAllStacks(originWindow)
    ...
    def openFilesAndAppendStacks(self, filePaths=None):
        if filePaths is None:
            filePaths = getOpenFileName(self,
                "Open image stacks", "", "Image files (*.tif *.tiff *.mat *.czi *.lsm)", multiFile=True
            )
        if len(filePaths) == 0:
            return
    ...
    def updateAllBranchesMinimalAngle(self, originWindow):
        """Update all primary branches, and show a status message while happening (as it's slow)"""
        originWindow.statusBar().showMessage("Updating all primary branches...")
        self.fullActions.updateAllBranchesMinimalAngle()
        originWindow.statusBar().clearMessage()
